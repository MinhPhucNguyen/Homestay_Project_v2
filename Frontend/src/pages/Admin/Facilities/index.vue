<template>
  ...
  <my-modal @clickTo="handleDeleteFacility" idModal="deleteConfirmModal" bgColor="danger">
  ...
  <div class="col-md-12 p-0">
    <div class="card">
      <div class="card-header bg-transparent d-flex align-items-center border-0">
        <div class="d-inline-block fw-bold text-dark fs-4 flex-grow-1">Danh sách tiện nghi</div>
        <div>
          <button class="btn btn-success fw-bold float-right ml-3" @click="addFacilities">
        ...
        <table class="table table-bordered table-striped">
          ...
          <tr v-for="facility in facilitiesList" :key="facility.facility_id">
            ...
            <td class="text-center">
              <img v-if="facility.facility_icon" :src="'data:image/svg+xml;utf8,' + facility.facility_icon" width="30"
                   height="30"/>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h1 class="modal-title fs-5" id="createFeatureModalLabel" v-else>Thêm tiện nghi</h1>
            <button
                type="button"
                class="btn-close"
          ...
          <div class="modal-body">
            <form @submit.prevent="facilitySubmit">
              <div class="form-group">
                ...
                <input type="text" class="form-control" v-model="model.facility_name"/>
                <small class="text-danger" v-if="errors.facility_name">{{ errors.facility_name[0] }}</small>
              </div>
              <div class="form-group">
              ...
              <div class="form-group" v-if="iconType === 'svg'">
                ...
                <textarea class="form-control" v-model="model.facility_icon" rows="4"></textarea>
              </div>
...
const getFacilitiesList = () => {
  return axios.get("v2/facilities").then((response) => {
    facilitiesList.value = response.data.data;
  });
};
...
const updateFacility = () => {
  isLoading.value = true;
  axios.post(`v2/facilities/${model.value.facility_id}/edit`, model.value, {params: {_method: "put"}}).then((response) => {
    successMessage.value = response.data.message;
    getFacilitiesList()
        .then(() => {
          isLoading.value = false;
...
const handleDeleteFacility = () => {
  isLoading.value = true;
  axios.delete(`v2/facilities/${model.value.facility_id}/delete`).then((response) => {
    successMessage.value = response.data.message;
    getFacilitiesList()
        .then(() => {
          isLoading.value = false;
